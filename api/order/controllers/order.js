...
const GUEST_ID = '62498007c79bdc14ec16c666'
...
module.exports = {
  async process(ctx) {
    const {
     ...
     const shippingOptions = [
      ...
      { label: "2-DAY SHIPPING", price: 5.99 },
  ...
  async finalize(ctx) {
      ...
      items.map(async (clientItem) => {
        ...
        if (clientItem.subscription) {
          ...
          await strapi.services.subscription.create({
            ...
            quantity: clientItem.qty, paymentMethod,
            shippingAddress,
            shippingInfo,
            billingAddress,
            billingInfo
    ...
    if(saveCard && ctx.state.user) {
      ...
      await strapi.plugins['users-permissions'].services.user.edit({ id: orderCustomer}, { paymentMethods: newMethods})
    }
    ...
    await strapi.plugins['email'].services.email.send({ 
      ...
      subject: 'DS-SPORT Order Confirmation',
      html: confirmation,
    })
  ...
  async removeCard(ctx) {
    ...
    const stripeMethods = await stripe.paymentMethods.list({ customer: stripeID, type: 'card'})
    ...
    const newUser = await strapi.plugins['users-permissions'].services.user.edit({ id: ctx.state.user.id }, { paymentMethods: newMethods })
