...
module.exports = {
    ...
    async delete(ctx) {
      ...
      async userFavorites(ctx) {
        let favorites = await strapi.services.favorite.find({ user: ctx.state.user.id})

        await Promise.all(
          favorites.map(async (favorite, i) => {
            const variants = await strapi.services.variant.find({ product: favorite.variant.product})
