  ...
  module.exports = {
      async setSettings(ctx) {
         ...
         if(typeof location !== "undefined" && typeof locationSlot !== "undefined") {
            newLocaitons[locationSlot] = location
         } else if (typeof location === 'undefined' && typeof locationSlot !== 'undefined') {
            newLocations[locationSlot] = { street: '', zip: '', city: '', state: ''}
         }

         let newUser = await strapi.plugins["users-permissions"].services.user.edit({ id }, { contactInfo: newInfo, locations: newLocaitons})
         ...
         async me(ctx) {
            ...
            newUser.favorites = favorites.map(favorite => ({ variant: favorite.variant.id, id: favorite.id}))
